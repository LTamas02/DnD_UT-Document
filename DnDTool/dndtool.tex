...
\begin{document}
	...
	\begin{lstlisting}[language=JavaScript,caption={Példa API végpont (szemléltető)}]
		...
		public IActionResult GetCampaigns() {
			// Lekérdezés és visszaadás DTO-ként
			return Ok(new { Items = new [] { "Campaign A", "Campaign B" } });
		}
	\end{lstlisting}
	...
	\begin{lstlisting}[language=JavaScript,caption={Példa React komponens (szemléltető)}]
		...
		export default function CampaignCard({ name, members }) {
			return (
			<div className="card">
			<h3>{name}</h3>
			<p>Játékosok: {members}</p>
			</div>
			);
		}
	\end{lstlisting}
	...
	\begin{itemize}
		...
		\item \textbf{Dobáskalkulátor:} testreszabható kockadobás
	...
	\begin{itemize}
		...
		\item Játékos karaktert készít, dobást indít, és chatben egyeztet.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{GET /api/campaigns}
		\item \texttt{POST /api/messages}
	\end{itemize}
